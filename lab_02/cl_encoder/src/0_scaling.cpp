...
void upload_to_GPU(Image *in){
  cl_int ret = clEnqueueWriteBuffer(com_qs[0], mem_R, CL_TRUE, 0, work_dim*sizeof(float), in->rc->data, 0, NULL, NULL);
  if(CL_SUCCESS != ret){
    report(FAIL, "clEnqueueWriteBuffer returned: %s (%d)", cluErrorString(ret), ret);
  }
  ret = clEnqueueWriteBuffer(com_qs[0], mem_G, CL_TRUE, 0, work_dim*sizeof(float), in->gc->data, 0, NULL, NULL);
  if(CL_SUCCESS != ret){
    report(FAIL, "clEnqueueWriteBuffer returned: %s (%d)", cluErrorString(ret), ret);
  }
  ret = clEnqueueWriteBuffer(com_qs[0], mem_B, CL_TRUE, 0, work_dim*sizeof(float), in->bc->data, 0, NULL, NULL);
  if(CL_SUCCESS != ret){
    report(FAIL, "clEnqueueWriteBuffer returned: %s (%d)", cluErrorString(ret), ret);
  }
  //clFinish(com_qs[0]);
...
void readback(Image *out){
    ...
    cl_int ret;
    ret = clEnqueueReadBuffer(com_qs[0], mem_R, CL_TRUE, 0, work_dim*sizeof(float), out->rc->data, 0, NULL, NULL);
    if(CL_SUCCESS != ret){
      report(FAIL, "clEnqueueReadBuffer returned: %s (%d)", cluErrorString(ret), ret);
    }
    ret = clEnqueueReadBuffer(com_qs[0], mem_G, CL_TRUE, 0, work_dim*sizeof(float), out->gc->data, 0, NULL, NULL);
    if(CL_SUCCESS != ret){
      report(FAIL, "clEnqueueReadBuffer returned: %s (%d)", cluErrorString(ret), ret);
    }
    ret = clEnqueueReadBuffer(com_qs[0], mem_B, CL_TRUE, 0, work_dim*sizeof(float), out->bc->data, 0, NULL, NULL);
    if(CL_SUCCESS != ret){
      report(FAIL, "clEnqueueReadBuffer returned: %s (%d)", cluErrorString(ret), ret);
    }
    //clFinish(com_qs[0]);
