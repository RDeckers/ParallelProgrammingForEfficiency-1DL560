...
void readback_blocking(cl_float *buffer){
    ...
    for(unsigned c = 0; c < work_dim; c++){
      if(c != buffer[c]){
        report(FAIL, "blocking failed at %u", c);
        exit(-1);
      }
    }
    //clFinish(com_qs[0]);
...
void readback_pin(cl_float *buffer){
  ...
  clEnqueueUnmapMemObject(com_qs[0], mem_A, mappedBuffer, 0, NULL, NULL);
  clFinish(com_qs[0]);
  if(mappedBuffer != buffer){
    report(FAIL, "Got a different pointer!");
    exit(-1);
  }
  if(CL_SUCCESS != ret){
    report(FAIL, "clEnqueueMapBuffer returned: %s (%d)", cluErrorString(ret), ret);
  ...
  for(unsigned c = 0; c < work_dim; c++){
    if(c != mappedBuffer[c]){
      report(FAIL, "map failed at %u, read %f", c, mappedBuffer[c]);
      exit(-1);
    }else{
      report(PASS, "%d", c);
    }
...
int encode() {
  ...
  size_t max_buffer_size_log2 = 16;
