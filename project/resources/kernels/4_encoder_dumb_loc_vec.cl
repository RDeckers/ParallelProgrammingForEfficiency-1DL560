...
float compute_block_delta(uint x_loc, uint y_loc, __local float* refference, __local float const *buffer){
  ...
  for(uint y = 0; y < 16; y++){
    ...
    for(uint x = 0; x < 16; x+=4){
      uint x_buf = x_loc+x;
      delta += fabs(vload4(x_buf+32*y_buf,buffer)-vload4(x+16*y,refference));
    }
  }
  barrier(CLK_LOCAL_MEM_FENCE);
...
void load_block(int block_x, int block_y, __global float const *global_source, __local float *local_buffer){
  ...
  uint glob_adress = (block_x*16+16*block_y*get_global_size(0))+get_local_id(0)+get_local_id(1)*get_global_size(0);
...
float4 motionVectorSearch_subroutine(
  ...
  __local float *tile_buffer //buffers for our tiles
    ...
    load_block(block_x-1, block_y-1,channel, tile_buffer);
    score[0] = compute_block_delta(x_loc, y_loc, ref_block, tile_buffer);

    load_block(block_x, block_y-1,channel, tile_buffer);
    score[1] = compute_block_delta(x_loc, y_loc,ref_block, tile_buffer);

    load_block(block_x-1, block_y,channel, tile_buffer);
    score[2] = compute_block_delta(x_loc, y_loc, ref_block, tile_buffer);

    load_block(block_x, block_y,channel, tile_buffer);
