...
#ifndef _WIN32
	#include <errno.h>
	//Define some wrappers for Microsofts 'secure' functions...
	#define sprintf_s(B, S, N, ...) snprintf(B, N, S, ##__VA_ARGS__)
	//http://stackoverflow.com/a/1513215/6019199
...
void dump_image(Image* image, const char *name, int frame_number) {
	...
	filename.append(to_string((_Longlong)frame_number));
	filename.append("-");
